...
class genericFactory
{
    /* The following is C++11's typedef for a pointer to a function!
     ...
     using BASE_CREATE_FN = std::shared_ptr<manufacturedObj> (*) (const std::string &par, const Point & p);
...
public:
    ...
    std::shared_ptr<manufacturedObj> create(const classIDKey &className,
						const std::string &param, const Point & xy ) const;
};
...
template <class manufacturedObj, typename classIDKey>
std::shared_ptr<manufacturedObj> genericFactory<manufacturedObj, classIDKey>::create(const classIDKey &className, const std::string &parms, const Point & xy  ) const
{
    std::shared_ptr<manufacturedObj> ret(nullptr);

    typename FN_REGISTRY::const_iterator regEntry = registry.find(className);
    if (regEntry != registry.end()) {
    	return (*regEntry).second(parms,xy);
    }
    return ret;
}
...
template <class ancestorType,
          ...
          typename classIDKey=defaultIDKeyType2 >
class registerInFactory{
public:
    static std::shared_ptr<ancestorType> createInstance(const std::string &pa ,  const Point & xy  )
    {
    	return std::shared_ptr<ancestorType>(manufacturedObj::createInstance(pa, xy));
    }
